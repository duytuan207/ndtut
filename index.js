  ...
  try {
    ...
    if (res.status === 200) {
      if (!lastStatuses[url] || lastStatuses[url].status !== res.status) {
        console.log(`[ OK ] ${url} đang hoạt động\n Tình trạng: ${res.status}\n còn sống: ${new Date().toISOString()}\n`);
      }
      lastStatuses[url] = {
        status: res.status,
      ...
      if (!lastStatuses[url] || lastStatuses[url].status !== res.status) {
        console.error(`[ERROR] ${url} trở về trạng thái ${res.status}`);
      }
      lastStatuses[url] = {
        status: res.status,
    ...
    if (!lastStatuses[url] || lastStatuses[url].status !== null) {
      console.error(`[ERROR] ${url} Không thể tiếp cận\n${error}`);
    }
    lastStatuses[url] = {
      status: null,
      uptime: lastStatuses[url] ? lastStatuses[url].uptime : 0
...
router.get('/', async (ctx) => {
  const responseData = {
    message: 'Uptime đã lên và đang chạy',
    urls: urls.map(url => {
      return {
        name: url.name,
        url: url.url,
...
console.log('====================================================');
console.log('      Duy Tuấn:Uptime Performance Monitoring          ');
console.log('====================================================');
